...
(
  select 
    vo1.visit_concept_id AS stratum1_id,
  ...
  (
		...
		join @cdmDatabaseSchema.observation_period op on vo.person_id = op.person_id
    where vo.visit_start_date <= op.observation_period_end_date
      and isnull(vo.visit_end_date,vo.visit_start_date) >= op.observation_period_start_date
